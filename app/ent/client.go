...
func (c *RecreationClient) MapCreateBulk(slice any, setFunc func(*RecreationCreate, int)) *RecreationCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &RecreationCreateBulk{err: fmt.Errorf("calling to RecreationClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*RecreationCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
// QueryUsers queries the users edge of a Recreation.
func (c *RecreationClient) QueryUsers(r *Recreation) *UserQuery {
...
// QueryRecreations queries the recreations edge of a User.
func (c *UserClient) QueryRecreations(u *User) *RecreationQuery {
...
type (
	hooks struct {
		Recreation, User []ent.Hook
	}
	inters struct {
		Recreation, User []ent.Interceptor
	}
)
